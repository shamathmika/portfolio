...
export default async function BlogPostPage({
  ...
  return (
    <article className="px-16 py-24">
      ...
      <h1 className="mt-1 max-w-3xl font-display text-4xl tracking-widest">{post.title}</h1>
      <div className="mt-10 flex max-w-2xl flex-col gap-8 font-subtitle text-lg leading-relaxed">
        {post.sections.map((section, i) => (
          <div key={i}>
            ...
            {section.body.map((paragraph, j) => (
              <p key={j} className="mb-4 last:mb-0 opacity-90">
                {paragraph}
              </p>
            ))}
          </div>
